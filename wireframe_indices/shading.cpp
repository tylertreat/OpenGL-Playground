...
//
// Similar to spinning cube example, but model is read using the 
// ObjFile code and indexed rendering is used.  The shader is 
// the simple depth shader from the spinning cube that uses the
// fragment z-value to modulate the color.
//
//
...
{  
  ...
  lightShader  = new Shader("vshader_depth.glsl", "fshader_depth.glsl");
